...
namespace
{
    struct MemberCapsule
    {
        ...
        struct FFTMemberStruct
        {
            ...
            std::unique_ptr<OTFFT::RFFT> rfft;
            std::unique_ptr<OTFFT::FFT> cfft;
            std::unique_ptr<OTFFT::DCT> dct;
            std::unique_ptr<OTFFT::Bluestein> bluestein;
        } fft;
    };
    ...
    public:
        ...
        void start()
        {
            ...
            m_capsule.fft.cfft = std::make_unique<OTFFT::FFT>(static_cast<int>(m_fft_size));
            m_capsule.fft.bluestein = std::make_unique<OTFFT::Bluestein>(static_cast<int>(m_fft_size));

            if ((m_fft_size & 1) == 0)
            {
                m_capsule.fft.rfft = std::make_unique<OTFFT::RFFT>(static_cast<int>(m_fft_size));
                m_capsule.fft.dct = std::make_unique<OTFFT::DCT>(static_cast<int>(m_fft_size));
            }
