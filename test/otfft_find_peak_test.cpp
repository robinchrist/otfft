...
namespace
{
    std::size_t findPeak(const std::size_t fft_length, const double sample_rate, const double signal_frequency)
    {
        ...
        OTFFT_Complex::simd_array<OTFFT_Complex::complex_t> spectrum(fft_length);
        {
            auto fft = std::make_unique<OTFFT::RFFT>(static_cast<int>(fft_length));
        ...
        std::size_t d = std::distance(&spectrum[0], peakPos);
